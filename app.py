...
st.set_page_config(page_title="Analyse de compétences et recommandations de carrière",
                   layout="wide")
st.title("Analyse de compétences et recommandations de carrière")

st.markdown(
    """
    Cette application analyse tes expériences, ton profil (études, expériences, projets)
    pour identifier tes compétences principales, propose des métiers cibles
    et générer des conseils personnalisés.
    """
)
    ...
    with col2:
        ...
        if not top_jobs:
            ...
            for i, item in enumerate(top_jobs, start=1):
                j = item["job"]
                score_pct = item["score"] * 100
                with st.expander(f"{i}. {j['title']} – adéquation estimée : {score_pct:.1f} %"):
